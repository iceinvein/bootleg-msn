...
describe('useOAuth', () => {
  beforeEach(() => {
    ...
    mockGetAuthErrorMessage.mockImplementation((error: any) => error.message || 'OAuth error');
  });
  ...
  describe('error handling', () => {
    ...
    it('should use centralized error message handling', async () => {
      ...
      expect(mockToastError).toHaveBeenCalledWith('User-friendly error message');
    });
  ...
  describe('authentication state updates', () => {
    it('should update when authentication state changes', () => {
      ...
      expect(renderHook(() => useOAuth()).result.current.isAuthenticated).toBe(false);
      ...
      expect(renderHook(() => useOAuth()).result.current.isAuthenticated).toBe(true);
    });
  });
