...
export function Message({ message }: MessageProps) {
	...
	return (
		<button
			type="button"
			className={`flex ${ownsMessage ? "justify-end" : "justify-start"} group`}
			onMouseEnter={() => setIsHovered(true)}
			onMouseLeave={() => setIsHovered(false)}
		>
			<div
				className={`relative flex max-w-[85%] items-end space-x-2 md:max-w-xs lg:max-w-md ${ownsMessage ? "flex-row-reverse" : ""}`}
			>
