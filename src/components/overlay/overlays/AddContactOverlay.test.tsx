...
vi.mock("@/components/ui/input", () => ({
  Input: ({ placeholder, onChange, value, ...props }: any) => (
    <input placeholder={placeholder} onChange={onChange} value={value} data-testid="input" {...props} />
  ),
}));

vi.mock("@/components/ui/textarea", () => ({
  Textarea: ({ placeholder, onChange, value, ...props }: any) => (
    <textarea placeholder={placeholder} onChange={onChange} value={value} data-testid="textarea" {...props} />
  ),
}));

vi.mock("@/components/ui/button", () => ({
  Button: ({ children, onClick, type, disabled, variant }: any) => (
    <button onClick={onClick} type={type} disabled={disabled} data-testid="button" data-variant={variant}>
      {children}
    </button>
  ),
}));

vi.mock("lucide-react", () => ({ UserPlus: () => <div />, Mail: () => <div />, User: () => <div /> }));
...
describe("AddContactOverlay", () => {
  ...
  it("renders header and description", () => {
    ...
    expect(screen.getByTestId("dialog-description")).toHaveTextContent("Send a contact request to connect with someone.");
  });
  ...
  it("submits contact request and shows success", async () => {
    ...
    await waitFor(() => {
      expect(mockSendContactRequest).toHaveBeenCalledWith({ contactEmail: "test@example.com", nickname: undefined });
      expect(vi.mocked(toast.success)).toHaveBeenCalled();
    });
  });

  it("switches to invitation mode when user not found", async () => {
    const mockSendContactRequest = vi.fn().mockRejectedValue(new Error("User not found"));
    mockUseMutation.mockReturnValueOnce(mockSendContactRequest);
    ...
    await waitFor(() => {
      expect(screen.getByTestId("dialog-title")).toHaveTextContent("Invite to MSN Messenger");
