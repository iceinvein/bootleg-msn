...
vi.mock("@/hooks/useOverlays", () => ({ useOverlays: () => ({ open: mockOpen }) }));
vi.mock("@/hooks/useAvatarUrls", () => ({ useGroupAvatarUrls: vi.fn(() => new Map()) }));
...
vi.mock("@/components/ui/avatar", () => ({ Avatar: ({ children }: { children: React.ReactNode }) => <div data-testid="avatar">{children}</div>, AvatarImage: ({ src }: { src: string }) => <img data-testid="avatar-image" src={src} /> }));
vi.mock("@/components/ui/button", () => ({ Button: ({ children, onClick }: any) => <button data-testid="button" onClick={onClick}>{children}</button> }));
vi.mock("lucide-react", () => ({ Info: () => <div />, Users: () => <div />, UserPlus: () => <div />, Pencil: () => <div /> }));
...
describe("GroupInfoOverlay", () => {
  ...
  it("opens Add Members overlay when clicking action", () => {
    render(<GroupInfoOverlay group={mockGroup} onClose={vi.fn()} />);
    const btn = screen.getAllByTestId("button").find(b => b.textContent?.includes("Add Members"))!;
    fireEvent.click(btn);
    expect(mockOpen).toHaveBeenCalledWith(expect.objectContaining({ type: "ADD_MEMBERS" }));
  });
});
