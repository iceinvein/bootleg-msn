...
import { getStatusColor, getStatusColorWithGlow, getMSNStatusIcon } from "../style";
...
describe("style utilities", () => {
	describe("getStatusColor", () => {
		...
		it("should handle all valid status types", () => {
			const validStatuses = ["online", "away", "busy", "invisible", "offline"] as const;
	...
	describe("getStatusColorWithGlow", () => {
		...
		it("should return colors without glow for inactive statuses", () => {
			expect(getStatusColorWithGlow("invisible")).toBe("bg-gray-400 border-gray-300");
			expect(getStatusColorWithGlow("offline")).toBe("bg-gray-400 border-gray-300");
		});

		it("should return default color for unknown status", () => {
			// @ts-expect-error - Testing invalid status
			expect(getStatusColorWithGlow("unknown")).toBe("bg-gray-400 border-gray-300");
		});
	...
	describe("integration tests", () => {
		it("should have consistent color schemes across all functions", () => {
			const statuses = ["online", "away", "busy", "invisible", "offline"] as const;
