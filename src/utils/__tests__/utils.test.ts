...
describe("lib/utils", () => {
	describe("cn (className utility)", () => {
		...
		it("should preserve non-conflicting classes", () => {
			expect(cn("text-red-500", "bg-blue-500", "p-4", "rounded")).toBe("text-red-500 bg-blue-500 p-4 rounded");
		});

		it("should handle custom CSS classes mixed with Tailwind", () => {
			expect(cn("custom-class", "bg-red-500", "another-custom")).toBe("custom-class bg-red-500 another-custom");
		});
