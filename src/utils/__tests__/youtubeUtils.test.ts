...
describe("youtubeUtils", () => {
	describe("extractYouTubeVideoId", () => {
		it("should extract video ID from standard YouTube URLs", () => {
			...
			expect(extractYouTubeVideoId("www.youtube.com/watch?v=dQw4w9WgXcQ")).toBe(
				"dQw4w9WgXcQ",
			);
		...
		it("should extract video ID from embed URLs", () => {
			...
			expect(extractYouTubeVideoId("youtube.com/embed/dQw4w9WgXcQ")).toBe(
				"dQw4w9WgXcQ",
			);
		});
		...
		it("should extract video ID from v/ URLs", () => {
			...
			expect(extractYouTubeVideoId("youtube.com/v/dQw4w9WgXcQ")).toBe(
				"dQw4w9WgXcQ",
			);
		});
	...
	describe("containsYouTubeUrl", () => {
		it("should return true for text containing YouTube URLs", () => {
			...
			expect(
				containsYouTubeUrl("Short link: https://youtu.be/dQw4w9WgXcQ"),
			).toBe(true);
			expect(
				containsYouTubeUrl("Embed: https://www.youtube.com/embed/dQw4w9WgXcQ"),
			).toBe(true);
		});

		it("should return true for URLs without protocol", () => {
			expect(
				containsYouTubeUrl("Check out: www.youtube.com/watch?v=dQw4w9WgXcQ"),
			).toBe(true);
		...
		it("should handle multiple URLs in text", () => {
			const text =
				"First video: https://youtu.be/abc123 and second: https://www.youtube.com/watch?v=def456";
			expect(containsYouTubeUrl(text)).toBe(true);
		});

		it("should be case insensitive", () => {
			expect(
				containsYouTubeUrl("Check out: YOUTUBE.COM/watch?v=dQw4w9WgXcQ"),
			).toBe(true);
			expect(containsYouTubeUrl("Short: YOUTU.BE/dQw4w9WgXcQ")).toBe(true);
		});
	});
	...
	describe("replaceYouTubeUrls", () => {
		...
		it("should replace multiple YouTube URLs with placeholders", () => {
			const text =
				"First: https://youtu.be/abc123 and second: https://www.youtube.com/watch?v=def456";
			const result = replaceYouTubeUrls(text);

			expect(result.text).toBe(
				"First: [YOUTUBE_VIDEO_0] and second: [YOUTUBE_VIDEO_1]",
			);
		...
		it("should handle URLs with parameters", () => {
			const text =
				"Video: https://www.youtube.com/watch?v=dQw4w9WgXcQ&t=30s&list=PLtest";
			const result = replaceYouTubeUrls(text);
		...
		it("should handle mixed content", () => {
			const text =
				"Text before https://youtu.be/abc123 text between https://www.youtube.com/watch?v=def456 text after";
			const result = replaceYouTubeUrls(text);

			expect(result.text).toBe(
				"Text before [YOUTUBE_VIDEO_0] text between [YOUTUBE_VIDEO_1] text after",
			);
			expect(result.videos).toHaveLength(2);
		});
	});
