...
const getRedirectUri = (platform: string, customRedirectUri?: string): string => {
...
describe('OAuth Utility Functions', () => {
  ...
  describe('generateOAuthUrl', () => {
    it('should generate Google OAuth URL for web platform', () => {
      ...
      expect(result).toContain('redirect_uri=https%3A%2F%2Ftest.example.com%2Foauth-callback');
    ...
    it('should generate Apple OAuth URL for mobile platform', () => {
      ...
      expect(result).toContain('redirect_uri=com.bootlegmsn.messenger%3A%2F%2Foauth-callback');
      expect(result).toContain('response_type=code');
    ...
    it('should use custom redirect URI when provided', () => {
      ...
      expect(result).toContain(`redirect_uri=${encodeURIComponent(customRedirectUri)}`);
    });
  ...
  describe('handleOAuthCallback', () => {
    it('should handle OAuth callback successfully', () => {
      const result = handleOAuthCallbackLogic('google', 'test-auth-code', 'test-state');
