...
const formatFileSize = (bytes: number): string => {
  ...
  const result = (bytes / Math.pow(k, i)).toFixed(1);
...
const createScheduledMessageValidator = v.object({
  content: v.string(),
...
describe("Backend Validation & Error Handling", () => {
  describe("Convex Validator Edge Cases", () => {
    it("should validate reaction types correctly", () => {
      const validReactions = ["thumbs_up", "heart", "laugh", "wow", "sad", "angry", "custom"];
    ...
    it("should validate scheduled message data structure", () => {
      ...
      expect(["text", "image", "file", "voice", "video"].includes(validData.messageType)).toBe(true);
      ...
      expect(["text", "image", "file", "voice", "video"].includes("invalid" as any)).toBe(false);
  ...
  describe("File Validation Edge Cases", () => {
    it("should validate voice message constraints", () => {
      // Valid voice message
      expect(() => validateVoiceMessage(1000000, "audio/mp3", 30)).not.toThrow();
      expect(() => validateVoiceMessage(5000000, "audio/wav", 60)).not.toThrow();

      // File too large
      expect(() => validateVoiceMessage(MAX_VOICE_MESSAGE_SIZE + 1, "audio/mp3", 30))
        .toThrow("Voice message too large");

      // Duration too long
      expect(() => validateVoiceMessage(1000000, "audio/mp3", MAX_VOICE_MESSAGE_DURATION + 1))
        .toThrow("Voice message too long");

      // Duration too short
      expect(() => validateVoiceMessage(1000000, "audio/mp3", MIN_VOICE_MESSAGE_DURATION - 1))
        .toThrow("Voice message too short");

      // Invalid format
      expect(() => validateVoiceMessage(1000000, "audio/invalid", 30))
        .toThrow("Unsupported audio format");

      // Edge case: exactly at limits
      expect(() => validateVoiceMessage(MAX_VOICE_MESSAGE_SIZE, "audio/mp3", MAX_VOICE_MESSAGE_DURATION))
        .not.toThrow();
      expect(() => validateVoiceMessage(1, "audio/mp3", MIN_VOICE_MESSAGE_DURATION))
        .not.toThrow();
    });

    it("should validate background image constraints", () => {
      // Valid background image
      expect(() => validateBackgroundImage(1000000, "image/jpeg")).not.toThrow();
      expect(() => validateBackgroundImage(2000000, "image/png")).not.toThrow();

      // File too large
      expect(() => validateBackgroundImage(MAX_BACKGROUND_IMAGE_SIZE + 1, "image/jpeg"))
        .toThrow("Background image too large");

      // Invalid format
      expect(() => validateBackgroundImage(1000000, "image/invalid"))
        .toThrow("Unsupported image format");

      // Edge case: exactly at limit
      expect(() => validateBackgroundImage(MAX_BACKGROUND_IMAGE_SIZE, "image/jpeg"))
        .not.toThrow();
  ...
  describe("Timezone Validation", () => {
    it("should validate timezone strings", () => {
      ...
      validTimezones.forEach(timezone => {
        ...
        expect(timezone.includes("/") || timezone === "UTC" || timezone === "GMT").toBe(true);
      });
  ...
  describe("ConvexError Handling", () => {
    ...
    it("should handle nested validation errors", () => {
      ...
      expect(complexData.email).toEqual(["Invalid email format", "Email already exists"]);
    });
  });
