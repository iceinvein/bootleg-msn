...
import { addReactionValidator, reactionTypeValidator } from "./validators";
...
export const addMessageReaction = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (message.groupId) {
			...
			if (!membership) {
				throw new Error(
					"Access denied: You must be a group member to react to group messages",
				);
			}
		} else if (
...
export const removeMessageReaction = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (message.groupId) {
			...
			if (!membership) {
				throw new Error(
					"Access denied: You must be a group member to remove reactions from group messages",
				);
			}
		} else if (
			message.senderId !== user._id &&
